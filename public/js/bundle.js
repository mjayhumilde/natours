(() => {
  ...
  var init_platform = __esm({
    "node_modules/axios/lib/platform/index.js"() {
      init_browser();
      init_utils2();
      platform_default = __spreadValues(__spreadValues({}, utils_exports), browser_default);
    }
  });
    ...
    "public/js/leaflet.js"() {
      displayMap = (locations) => {
        const map = L.map("map", {
          ...
          // start zoomed out
          center: [0, 0]
          // arbitrary center before flying in
        });
        L.tileLayer(
          "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png",
          {
            attribution: '&copy; <a href="https://carto.com/">CARTO</a> | \xA9 OpenStreetMap contributors',
            subdomains: "abcd",
            maxZoom: 19
          }
        ).addTo(map);
        ...
        locations.forEach((loc) => {
          ...
          L.marker([loc.coordinates[1], loc.coordinates[0]], { icon: customIcon }).addTo(map).bindPopup(`<p>Day ${loc.day}: ${loc.description}</p>`, {
            autoClose: false,
            closeonClick: false,
            offset: [0, -40]
          }).openPopup();
      ...
      updateSettings = (data, type) => __async(null, null, function* () {
        try {
          ...
          if (res.data.status === "success") {
            showAlert(
              "success",
              `${type === "password" ? "Password " : "Infomation"} Updated Successfully!`
            );
            window.setTimeout(() => {
              location.assign("/me");
            }, 1e3);
  ...
  var require_index = __commonJS({
    "public/js/index.js"(exports) {
      ...
      if (bookTourButton) {
        bookTourButton.addEventListener("click", (e) => __async(null, null, function* () {
